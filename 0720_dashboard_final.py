...
@st.cache_data
def load_data():
    # ⚠️ 구글 드라이브에 올려둔 parquet 파일의 ID로 확인해주세요!
    file_id = "11apg9KTfsAtATCnQlSSpuXlTCPoAiNPM"  
    local_path = "data_final.parquet"
    
    # 1. 파일이 없거나, 깨진/잘못 다운로드된 캐시 파일(예: 1MB 미만) 삭제
    if os.path.exists(local_path):
        if os.path.getsize(local_path) < 1000000: # 1MB 미만시 삭제 후 재다운로드
            os.remove(local_path)

    # 2. 파일이 없으면 구글 드라이브에서 다운로드
    if not os.path.exists(local_path):
        download_file_from_google_drive(file_id, local_path)
    
    # 3. Parquet 읽기 (CSV와 달리 인코딩 시도 필요 없음)
