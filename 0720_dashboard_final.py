...
# 2. 데이터 불러오기 함수 (구글 드라이브 대용량 우회 적용)
# 2. 데이터 불러오기 함수 (구글 드라이브 대용량 다운로드 우회 완벽 적용)
@st.cache_data
def load_data():
    file_id = "11apg9KTfsAtATCnQlSSpuXlTCPoAiNPM"
    local_path = "data_final.csv"
    
    # 1. 혹시 이전 실패로 잘못 생성된 HTML 파일이 남아있다면 삭제
    if os.path.exists(local_path):
        # 파일 크기가 너무 작으면(HTML 페이지일 확률 높음) 삭제 후 재다운로드
        if os.path.getsize(local_path) < 1000000:  # 1MB 미만이면 깨진 파일
            os.remove(local_path)

    # 2. 파일이 없을 때 진짜 CSV 우회 다운로드 실행
    if not os.path.exists(local_path):
        # fuzzy=True 옵션을 넣어야 구글 드라이브 바이러스 경고창을 자동 통과합니다.
        url = f"https://drive.google.com/uc?id={file_id}"
        gdown.download(url, local_path, quiet=False, fuzzy=True)
    
    # 3. CSV 데이터 읽기 (인코딩 예외 처리)
    try:
...
try:
    ...
    st.error(f"구글 드라이브에서 CSV 파일을 불러오는 중 오류가 발생했습니다. 링크 공유 설정 및 gdown 설치 상태를 확인해 주세요. 오류: {e}")
    st.stop()
