...
# 2. 데이터 불러오기 함수 (gdown 최신 호환성 수정)
@st.cache_data
def load_data():
    file_id = "11apg9KTfsAtATCnQlSSpuXlTCPoAiNPM"
    local_path = "data_final.csv"
    
    # 1. 혹시 이전 실패로 잘못 생성된 HTML/깨진 파일이 있다면 삭제
    if os.path.exists(local_path):
        # 파일 크기가 1MB 미만이면 깨진 파일(HTML 경고창 등)로 판단하고 삭제
        if os.path.getsize(local_path) < 1000000:
            os.remove(local_path)

    # 2. 파일이 없으면 구글 드라이브에서 직접 다운로드
    if not os.path.exists(local_path):
        # 대용량/바이러스 스캔 경고창을 우회하는 직링크 방식
        url = f"https://drive.google.com/uc?export=download&id={file_id}"
        gdown.download(url, local_path, quiet=False)
    
    # 3. CSV 데이터 읽기 (인코딩 자동 시도)
    try:
